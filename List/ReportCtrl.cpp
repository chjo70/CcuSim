...
//##ModelId=43D481990274
struct ROWINFO
{
	//##ModelId=43D481990329
	DWORD dwData;
	//##ModelId=43D4819903E7
	DWORD dwStates;
	//##ModelId=43D4819A00A9
	CArray<int, int> aImages;
	//##ModelId=43D4819A0154
	CStringArray aTexts;
};
...
{
	return date.ParseDateTime(str, VAR_TIMEVALUEONLY);
}
...
CReportCtrl::CReportCtrl(): m_pWndEdit(NULL)
{
	...
	m_pTextTip = new CToolTipCtrlTrack;
}
    ...
    else if (lplvcd->nmcd.dwDrawStage == (CDDS_ITEMPREPAINT | CDDS_SUBITEM))
    {
		...
		if( p != NULL ) {
			...
			*pResult = CDRF_DODEFAULT;
			...
			//m_pTextTip->SetTrackToolTip( "AAA", 0, 0 );
		}
	}
}
...
//////////////////////////////////////////////////////////////////////
//
// 함 수 이 름  : CReportCtrl::OnNotify
// 반환되는 형  : BOOL
// 함 수 인 자  : WPARAM wParam
// 함 수 인 자  : LPARAM lParam
// 함 수 인 자  : LRESULT* pResult
// 함 수 설 명  : 리스트뷰 컨트롤안의 헤더 컨트롤은 컬럼크기를 바꾸기
//								(Resizing)전에 자신의 부모 윈도우에게 통지(notification)
//								를 보냅니다. 우리는 CListCtrl 파생 클래스에서 이 통지 
//								메시지를 처리하기위해 OnNotify() 함수를 오버라이드 
//								할수있습니다.
//								http://myku.hihome.com/
// 최 종 변 경  : 조철희, 2004-09-07 23:14:41
//
...
//////////////////////////////////////////////////////////////////////
//
// 함 수 이 름  : CReportCtrl::OnEndlabeledit
// 반환되는 형  : void
// 함 수 인 자  : NMHDR* pNMHDR
// 함 수 인 자  : LRESULT* pResult
// 함 수 설 명  : 편집후에 자동적으로 재정렬 하기
// 최 종 변 경  : 조철희, 2004-09-07 23:37:18
//
//##ModelId=43D481870047
void CReportCtrl::OnEndlabeledit(NMHDR* pNMHDR, LRESULT* pResult) 
{
	// TODO: Add your control notification handler code here
...
int CReportCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	if( ! m_pTextTip->Create(this) ) {
		TRACE("Unable To create ToolTip\n");
		return FALSE;
	}
	m_pTextTip->Activate( TRUE );
//	m_pTextTip->InitToolTip( this, "ABC" );
...
void CReportCtrl::SortItems(int nColumn, BOOL bAscending)
{
	...
	do
	{
		...
		{
			nSep2 = GetItemCount();
			// 에미터 번호가 널문자일때까지 카운트한다.
			// 그래서 정렬하는데 널문자를 뺀 열까지 해서 보기 편하게 한다.
			//-- 조철희 2004-09-07 11:15:29 --//
			/*
			for( int i=0 ; i < nSep2 ; ++i ) {
				CString strBuff;

				strBuff = GetItemText( i, 0 );
				if( strBuff.IsEmpty() )
					break;
			}			
			nSep2 = i;*/
...
/*
	...
	BOOL bret1, bret2;
	bret1 = date1.ParseDateTime(s1, VAR_TIMEVALUEONLY);
	throw( CMemoryException );
	
	// bret2 = date2.ParseDateTime(s2, VAR_TIMEVALUEONLY);

	//-- 조철희 2004-09-22 09:37:57 --//
	//_ITEM_COMPARE_FUNCS::_IsDate(s1, date1);
	//_ITEM_COMPARE_FUNCS::_IsDate(s2, date2);
